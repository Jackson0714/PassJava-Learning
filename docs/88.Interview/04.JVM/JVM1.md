...
另外如果 YGC 后，存活的对象超过了 Survivor 的 50%，则会进入老年代，我们的**调优思路**是尽量减少对象进入老年代，以减少发生 FGC 的频率。所以通过调整 Eden 区的大小，减少了对象进入老年代的频率。
...
## 十一、美团一面：有在⼯作时间中使⽤过 jstat, jmap, mat⼯具吗？ 能给⼀个实际的例⼦说明⼀下吗？
...
printf “%x\n” <pid>，把线程 pid 转为16进制，比如 0xf58
jstack pid | g rep -A 10 0xf58 查看线程的堆栈日志
```
