...
### 1.7 CopyOnWriteArrayList底层源码分析

#### 1.7.1 用积木模拟CopyOnWriteArrayList的add操作

![CopyOnWriteArrayList的add操作](http://cdn.jayh.club/blog/20200828/175709345.png)

当往CopyOnWriteArrayList存放元素时，给盒子加了一个锁，只有一个积木可以进行存放，放完后，释放锁，放第二元素时，再进行加锁，依次往复进行。
