...
![18个Queue的继承实现关系图](http://cdn.jayh.club/blog/20200906/Rd2SndMwKpaN.png?imageslim)
...
- （2）同理移除（Remove）元素的动作，队列为空时，remove方法抛异常，而poll返回null。如果移除头部的元素成功，则返回移除的元素。

- （3）同理检测（Examine）元素的动作，返回头部元素，但不删除元素， 如果队列为空，则element()方法抛异常，而peek()返回false。
