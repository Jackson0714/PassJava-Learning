# ELK 别只会搜日志，求你懂点搜索引擎吧！

# 前言

项目中我们总是用 `Kibana`  来搜索日志，看下有没有异常信息。`Kibana` 就是 我们常说的 `ELK` 中的 `K`。

如下图所示：

![mark](http://cdn.jayh.club/blog/20201007/oxihEk0lT425.png?imageslim)

但这些日志是如何被检索出来的呢？这里就该我们的 Elasticsearch 搜索引擎登场了。

# 一、Elasticserach 简介

## 1.1 什么是 Elasticserach?
...
这里我们可以自己搭建一套 nginx 环境，然后把ikwords.text 放到 nginx 根目录。
