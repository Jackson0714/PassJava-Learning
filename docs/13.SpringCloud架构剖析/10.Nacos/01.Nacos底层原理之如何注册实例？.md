...
- **随机节点**：客户端随机选择集群中的一个 Nacos 节点发起注册。
...
- **异步重试**：如果注册失败，客户端将会切换 Nacos 节点，再次发起注册请求。
...
上面我们讲到，Nacos 的有多个节点，当节点发现这个请求不是属于自己的，就会进行转发。如下图所示：
...
**没看懂的点**：我这里启动了三个 Nacos 节点，如下图所示，但是为什么会多了一个 192.168.10.197:8848 的节点？

![](http://cdn.jayh.club/uPic/image-202204122033417675s0J4F.png)
...
## 问题 2：Nacos 如何接收注册请求的？

## 问题 3：Nacos 对写请求是如何进行路由转发的？
...
![](http://cdn.jayh.club/uPic/image-20220410110704520KkW9ZoIveTcF.png)
